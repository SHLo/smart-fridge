...
static void getTelemetryPayload(az_span payload, az_span* out_payload)
{
  ...
  data["a"] = analogRead(gas1Pin);
  data["b"] = analogRead(gas2Pin);
  
  serializeJson(data, json_str);
  payload = az_span_copy(payload, az_span_create_from_str(json_str));
  Logger.Info(String("a:") + String(analogRead(gas1Pin)));
  Logger.Info(String("b:") + String(analogRead(gas2Pin)));
